...
export const CONFIG = {
  ...
  telegramChannel2UserName: (process.env.TELEGRAM_CHANNEL2_USERNAME ?? "").trim().toLowerCase(),
  telegramChannel2Id: process.env.TELEGRAM_CHANNEL2_ID,
  ...
  maxOpenPositions: number("MAX_OPEN_POSITIONS", 999),

  stopLossPct: number("STOP_LOSS_PERCENT", -25) / 100,
  takeProfitPct: number("TAKE_PROFIT_PERCENT", 500) / 100,
  trailingActivationPct: number("TRAILING_ACTIVATION_PERCENT", 20) / 100,
  trailingStopPct: number("TRAILING_STOP_PERCENT", 12) / 100,

  baseTtlSecs: number("BASE_TTL_SECS", 3600),
  maxTtlSecs: number("MAX_TTL_SECS", 10800),
  ...
  pricePollIntervalMs: number("PRICE_POLL_INTERVAL_MS", 200),
